...
FF.registerUI("actionResult", function(obj,app,scope)
{
    ...
    var main = $("<div class='flexrow'>");
    for(let i = 0; i < scope.action.length; i++)
    {
        let div = $("<div class='flex'>").appendTo(main);
        if(i + 1 < scope.action.length)
        {
            div.css("border-right","1px solid rgba(0,0,0,0.05)")
        }
        let action = scope.action[i];
        if(action.attack)
        {
